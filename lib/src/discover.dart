...
Future<List<HolonEntry>> _discoverInRoot(String root, String origin) async {
  ...
  void scan(Directory current) {
    ...
    for (final entity in entities) {
      ...
      if (entity is! File || name != 'holon.yaml') {
        continue;
      }

      final dir = _dirname(path);
      try {
        final identity = parseHolon(path);
        final manifest = _parseManifest(path);
...
HolonManifest _parseManifest(String path) {
  ...
  final build = _mapValue(raw['build']);
  final artifacts = _mapValue(raw['artifacts']);
...
YamlMap _mapValue(Object? value) {
  if (value is YamlMap) {
    return value;
  }
  return YamlMap();
