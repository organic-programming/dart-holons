...
class MemRuntimeConnection implements RuntimeConnection {
  MemRuntimeConnection({
    required StreamController<List<int>> incoming,
    required StreamController<List<int>> outgoing,
  })  : _incoming = incoming,
        _outgoing = outgoing,
        _iterator = StreamIterator<List<int>>(incoming.stream);
  ...
  Future<void> write(List<int> data) async {
    ...
    if (_outgoing.isClosed) {
      throw StateError('peer is closed');
    }
    _outgoing.add(List<int>.from(data));
  }
...
class MemRuntimeListener implements RuntimeTransportListener {
  MemRuntimeListener({this.name = ''});

  final String name;
  final Queue<MemRuntimeConnection> _pending = Queue<MemRuntimeConnection>();
  final Queue<Completer<MemRuntimeConnection>> _waiters = Queue<Completer<MemRuntimeConnection>>();
  bool _closed = false;

  @override
  String get boundUri => name.isEmpty ? 'mem://' : 'mem://$name';

  Future<RuntimeConnection> dial() async {
    if (_closed) {
      throw StateError('listener closed: $boundUri');
    }
    ...
    final client = MemRuntimeConnection(incoming: toClient, outgoing: toServer);
    final server = MemRuntimeConnection(incoming: toServer, outgoing: toClient);

    if (_waiters.isNotEmpty) {
      _waiters.removeFirst().complete(server);
    } else {
      _pending.add(server);
    }
  ...
  Future<RuntimeConnection> accept() async {
    ...
    final waiter = Completer<MemRuntimeConnection>();
    _waiters.add(waiter);
  ...
  Future<void> close() async {
    ...
    while (_pending.isNotEmpty) {
      await _pending.removeFirst().close();
    }

    while (_waiters.isNotEmpty) {
      final waiter = _waiters.removeFirst();
      if (!waiter.isCompleted) {
        waiter.completeError(StateError('listener closed: $boundUri'));
      }
    }
  }
}
...
ParsedUri parseUri(String uri) {
  ...
  switch (s) {
    ...
    case 'mem':
      final raw = uri.startsWith('mem://') ? uri : 'mem://';
      final name = raw.substring('mem://'.length);
      return ParsedUri(raw: raw, scheme: 'mem', path: name);
    case 'ws':
    case 'wss':
      final secure = s == 'wss';
...
Future<TransportListener> listen(String uri) async {
  ...
  switch (parsed.scheme) {
    ...
    case 'mem':
      return MemTransportListener(address: parsed.raw);
    case 'ws':
    case 'wss':
...
Future<RuntimeTransportListener> listenRuntime(String uri) async {
  ...
  switch (parsed.scheme) {
    ...
    case 'mem':
      return MemRuntimeListener(name: parsed.path ?? '');
    case 'ws':
    case 'wss':
