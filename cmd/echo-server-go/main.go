...
const (
	...
	// If SIGTERM arrives immediately after startup, allow a brief settle window
	// so already-launched clients can finish connecting before graceful stop.
	shutdownStartupWindow = 2 * time.Second
	shutdownStartupGrace  = 700 * time.Millisecond
...
func shutdown(server *grpc.Server, startedAt time.Time) {
	if time.Since(startedAt) < shutdownStartupWindow {
