...
import '../gen/holonmeta/v1/holonmeta.pb.dart';
import '../gen/holonmeta/v1/holonmeta.pbgrpc.dart';
import 'identity.dart';

const _holonMetaService = 'holonmeta.v1.HolonMeta';

final RegExp _packagePattern = RegExp(r'^package\s+([A-Za-z0-9_.]+)\s*;');
final RegExp _servicePattern = RegExp(r'^service\s+([A-Za-z_][A-Za-z0-9_]*)\s*\{?');
final RegExp _messagePattern = RegExp(r'^message\s+([A-Za-z_][A-Za-z0-9_]*)\s*\{?');
...
final RegExp _enumValuePattern = RegExp(r'^([A-Za-z_][A-Za-z0-9_]*)\s*=\s*(-?\d+)\s*;');
...
DescribeResponse buildDescribeResponse({
  ...
  required String holonYamlPath,
}) {
  final identity = parseHolon(holonYamlPath);
  final index = _parseProtoDirectory(protoDir);

  return DescribeResponse()
    ..slug = _slugFor(identity)
...
HolonMetaDescribeService describeService({
  ...
  return HolonMetaDescribeService._(
    buildDescribeResponse(protoDir: protoDir, holonYamlPath: holonYamlPath),
  );
}
...
ServiceDoc _serviceDoc(_ServiceDef service, _ProtoIndex index) {
  return ServiceDoc()
    ...
    ..methods.addAll(service.methods.map((method) => _methodDoc(method, index)));
}
        ...
        if (rpcMatch != null && index.services.isNotEmpty) {
          index.services.last.methods.add(
            _MethodDef(
              ...
              inputType: _resolveTypeName(rpcMatch.group(3)!, packageName, const <String>[], index),
              outputType: _resolveTypeName(rpcMatch.group(5)!, packageName, const <String>[], index),
        ...
        if (mapFieldMatch != null) {
          index.messages[key]?.fields.add(
            _FieldDef(
              ...
              type: null,
              mapKeyType: _resolveTypeName(mapFieldMatch.group(2)!, packageName, scope, index),
              mapValueType: _resolveTypeName(mapFieldMatch.group(3)!, packageName, scope, index),
        ...
        if (fieldMatch != null) {
          ...
          index.messages[key]?.fields.add(
            _FieldDef(
              ...
              type: _resolveTypeName(fieldMatch.group(2)!, packageName, scope, index),
              mapKeyType: null,
              mapValueType: null,
              packageName: packageName,
              scope: scope,
...
String _slugFor(HolonIdentity identity) {
  final given = identity.givenName.trim();
...
String _resolveTypeName(
  ...
  for (var i = scope.length; i >= 0; i--) {
    final candidate = _qualify(packageName, _qualifyScope(scope.take(i).toList(), cleaned));
